...
* LMS is a JAVA Spring boot application built and deployed using the Jenkins pipeline.[See pipeline script from here](srcs/spring-pipeline.md). This project has a DevSecOPS feature where I implemented hashicorp vault for credential management, trivy for vulnerability scan, AVScan, and ModSecurity. Every stage in the pipeline was executed using the Jenkins shared library.
